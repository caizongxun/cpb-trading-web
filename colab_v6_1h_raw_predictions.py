...
# -*- coding: utf-8 -*-
"""
V6 1H Only Visualization - Raw Predictions (No Smoothing)
...
print("="*80 + "\nV6 1H Visualization (Raw Predictions - No Smoothing)\n" + "="*80 + "\n")

print("[Step 1] Installing dependencies...")
subprocess.run(['pip', 'install', 'tensorflow', 'huggingface_hub', '-q'], check=False)
print("✓ Dependencies installed\n")

print("[Step 2] Downloading V6 models...")
from huggingface_hub import hf_hub_download
model_cache, scalers_cache, model_stats = {}, {}, {'v6': 0, 'synthetic': 0}

def get_v6_model_1h(symbol):
    """Download V6 1h model with scalers"""
    model_key = f"{symbol}_{TIMEFRAME}"
    
    if model_key in model_cache:
        return model_cache[model_key], model_stats['v6']
    
    print(f"      {symbol}_1h_model.h5...", end=' ', flush=True)
    try:
        model_path = hf_hub_download(
            repo_id="zongowo111/cpb-models",
            filename=f"models_v6/{symbol}_1h_model.h5",
            repo_type="dataset",
        ...
        try:
            scalers_path = hf_hub_download(
                repo_id="zongowo111/cpb-models",
                filename=f"models_v6/{symbol}_1h_scalers.pkl",
                repo_type="dataset",
                cache_dir="./models_cache"
            )
        ...
        print("✗ Not found, using Synthetic")
        model_stats['synthetic'] += 1
        return None, False

print("✓ V6 Model loader ready\n")
...
def generate_predictions_raw(data, model_path=None, scalers_path=None):
    """Generate raw predictions - NO smoothing"""
...
def create_visualization(symbol, data, predictions, has_model=False):
    ...
    model_label = "V6" if has_model else "SYNTHETIC"
    ...
    stats_text = f"Model: {model_label} | MAE: {mae:.4f} | RMSE: {rmse:.4f} | MAPE: {mape:.2f}% | Raw (No Smoothing)"
    ...
    filename = f"v6_1h_raw_{symbol}_{timestamp}.png"
...
print("[Step 4] Generating V6 1H visualizations (Raw Predictions)...")
...
for i, symbol in enumerate(SYMBOLS, 1):
    ...
    try:
        ...
        model_path, has_model = get_v6_model_1h(symbol)
...
print(f"\n{'='*80}")
print(f"\n[Step 5] Summary")
print(f"Generated: {generated}/{len(SYMBOLS)} charts (1H only)")
print(f"Success Rate: {(generated/len(SYMBOLS))*100:.1f}%\n")
print(f"Model Usage:")
print(f"  V6 Models:  {model_stats['v6']}")
print(f"  Synthetic:  {model_stats['synthetic']}\n")
print(f"Prediction Method: RAW - NO SMOOTHING\n")
for symbol, tf, has_model, filename in results:
    status = "✓ V6" if has_model else "○ SYN"
    print(f"  {symbol} 1H  {status}  {filename}")

print(f"\n{'='*80}")
print(f"\n[Step 6] Displaying V6 1H charts...\n")

try:
    from IPython.display import Image, display
    import glob
    
    chart_files = sorted(glob.glob('v6_1h_raw_*.png'))
    if chart_files:
...
print(f"\n{'='*80}")
print(f"\n✓ Complete! Generated {generated} V6 1H visualizations!")
print(f"\nKey Features:")
print(f"  ✅ V6 Model")
print(f"  ✅ Raw Predictions (No Smoothing)")
print(f"  ✅ Practical Trading Use")
print(f"{'='*80}\n")
