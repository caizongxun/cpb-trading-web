...
# -*- coding: utf-8 -*-
"""
Visualize All V5 Model Predictions (Colab Version)

Generate visualizations for all trading pairs and timeframes using V5 models
Compare real prices vs predicted prices for all V5 models
...
Usage in Colab:
  ...
  !python colab_visualize_v5_all.py
"""
...
HF_REPO_ID = "zongowo111/cpb-models"  # Same repo, but v5 models inside
REPO_TYPE = "dataset"
VIS_CANDLES = 100

SYMBOLS = ['BTC', 'ETH', 'BNB', 'ADA', 'SOL', 'XRP', 'DOGE', 'ATOM']
TIMEFRAMES = ['15m', '1h', '4h', '1d']
MODEL_VERSION = 'v5'  # V5 models
...
def print_header():
    print("\n" + "="*80)
    print("V5 Model Prediction Visualizer - All Pairs & Timeframes")
    print("="*80 + "\n")
...
def download_v5_model(symbol, timeframe, token):
    """
    Download V5 model from HuggingFace
    
    V5 models are stored in models_v5 directory
    Fallback to v6 if v5 not available
    """
    try:
        from huggingface_hub import hf_hub_download
        
        # Try V5 first
        filename_v5 = f"models_v5/{symbol}_{timeframe}.keras"
        try:
            model_path = hf_hub_download(
                repo_id=HF_REPO_ID,
                filename=filename_v5,
                repo_type=REPO_TYPE,
                cache_dir="./models_cache",
                token=token
            )
            print(f"    ✓ V5 model found")
            return model_path, 'v5'
        except:
            pass
        
        # Fallback to V6 if V5 not available
        filename_v6 = f"models_v6/{symbol}_{timeframe}.keras"
        try:
            model_path = hf_hub_download(
                repo_id=HF_REPO_ID,
                filename=filename_v6,
                repo_type=REPO_TYPE,
                cache_dir="./models_cache",
                token=token
            )
            print(f"    ⚠ V5 not found, using V6 instead")
            return model_path, 'v6'
        except:
            pass
...
def generate_predictions(data, model_path=None):
    """Generate predictions using V5 or fallback model"""
    ...
    if model_path:
        try:
            ...
            # Try different lookback windows for V5 models
...
def create_visualization(symbol, timeframe, data, predictions, model_version='v5'):
    ...
    filename = f"prediction_v5_viz_{symbol}_{timeframe}_{timestamp}.png"
...
def main():
    ...
    # ========================================================================
    print_section("Step 2: Generate All V5 Model Visualizations")
    ...
    for symbol in tqdm(SYMBOLS, desc="Symbols", position=0):
        for timeframe in tqdm(TIMEFRAMES, desc=f"  {symbol}", position=1, leave=False):
            try:
                ...
                # Try to download V5/V6 model
                model_path = None
                model_version_used = 'synthetic'
                
                if token:
                    model_path, model_version_used = download_v5_model(symbol, timeframe, token)
                    if model_version_used:
                        model_stats[model_version_used] += 1
                    else:
                ...
                # Create visualization
                filename = create_visualization(symbol, timeframe, data, predictions, model_version_used)
                
                results.append({
                    'symbol': symbol,
                    'timeframe': timeframe,
                    'model': model_version_used,
                    'filename': filename,
                    'status': '✓',
                })
                generated += 1
    ...
    print(f"\n  Model Usage:")
    print(f"    V5 Models:  {model_stats['v5']} used")
    print(f"    V6 Models:  {model_stats['v6']} used (fallback)")
    print(f"    Synthetic:  {model_stats['synthetic']} used (no model available)")
    ...
    for symbol in SYMBOLS:
        ...
        if symbol_results:
            ...
            for r in symbol_results:
                ...
                model = r['model'].upper().ljust(8)
                print(f"    [{status}] {tf} [{model}] {r['filename'] if r['filename'] else 'Failed'}")
    ...
    try:
        ...
        viz_files = sorted(glob.glob('prediction_v5_viz_*.png'))
    ...
    print(f"\n  ✓ All V5 model visualizations completed!")
    print(f"\n  Statistics:")
